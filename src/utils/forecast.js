...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url, json: true }, (error, { body }) => { //proxy parameter is needed after 'json' parameter if working on office network
        if (error) {
            ...
            callback(undefined, body.daily.data[0].summary + 'Highest temperature is ' + body.daily.data[0].temperatureHigh + ' It is currently ' + body.currently.temperature + ' degress out. There is a ' + body.currently.precipProbability + '% chance of rain.')
        }
    })
}
